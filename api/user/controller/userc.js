...
module.exports.login = async(req,res)=>{
    try{
       let refreshtoken_user=jwt.sign({username:req.body.name},req.cookies["_csrf"],{expiresIn: "2h"})
    ...
    await db.user.findAll({where:{key:key_id},raw:true}).then((data)=>{
        if(data.length==1){
            ...
            if(data[0].refreshtoken==req.signedCookies["refresh_token"]){
                ...
                let access_token=jwt.sign({role:data.role,uid:data.id,username:data.name},framework.jwtkey,{expiresIn: "1h"})
                let refresh_token=jwt.sign({uid:data.id},req.cookies["_csrf"],{expiresIn: "2h"})
